...
std::shared_ptr<settings> cg::settings::parse_settings(int argc, char** argv)
{
	...
	add_options("camera_position", "Camera position", cxxopts::value<std::vector<float>>()->default_value("2.0,1.5,3.0"));
	add_options("camera_theta", "Camera polar angle", cxxopts::value<float>()->default_value("320.0"));
	add_options("camera_phi", "Camera azimuth angle", cxxopts::value<float>()->default_value("10.0"));
	...
	add_options("raytracing_depth", "Maximum number of traces rays", cxxopts::value<unsigned>()->default_value("10"));
	add_options("accumulation_num", "Number of accumulated frames", cxxopts::value<unsigned>()->default_value("20"));
